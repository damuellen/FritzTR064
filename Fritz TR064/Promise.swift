...
public class Promise<Value, Error> {
  private let source: FutureSource<Value, Error>
  ...
  public convenience init(value: Value) {
    self.init(source: FutureSource(state: .Resolved(value), originalSource: nil, warnUnresolvedDeinit: false))
  }
  ...
  public convenience init(error: Error) {
    self.init(source: FutureSource(state: .Rejected(error), originalSource: nil, warnUnresolvedDeinit: false))
  }

  internal init(source: FutureSource<Value, Error>) {
    self.source = source
  }
  ...
  public var result: Future<Value, Error>? {
    switch source.state {
  ...
  /// Or, when Promise is resolved later on, the handler is called synchronously on the thread where `FutureSource.resolve` is called.
  public func then(handler: Value -> Void) -> Promise<Value, Error> {

    let resultHandler: Future<Value, Error> -> Void = { result in
      switch result {
  ...
  /// Or, when Promise is rejected later on, the handler is called synchronously on the thread where `FutureSource.reject` is called.
  ...
  /// the handler is called synchronously on the thread where `FutureSource.resolve` or `FutureSource.reject` is called.
  public func finally(handler: () -> Void) -> Promise<Value, Error> {

    let resultHandler: Future<Value, Error> -> Void = { _ in
      handler()
    }
  ...
  /// Or, when Promise is resolved or rejected later on,
  /// the handler is called synchronously on the thread where `FutureSource.resolve` or `FutureSource.reject` is called.
  public func finallyResult(handler: Future<Value, Error> -> Void) -> Promise<Value, Error> {
  ...
  public func map<NewValue>(transform: Value -> NewValue) -> Promise<NewValue, Error> {
    let resultSource = FutureSource<NewValue, Error>(state: .Unresolved, originalSource: self.source, warnUnresolvedDeinit: true)

    let handler: Future<Value, Error> -> Void = { result in
      switch result {
      case .Value(let value):
  ...
  public func flatMap<NewValue>(transform: Value -> Promise<NewValue, Error>) -> Promise<NewValue, Error> {
    let resultSource = FutureSource<NewValue, Error>()
  ...
  public func mapError<NewError>(transform: Error -> NewError) -> Promise<Value, NewError> {
    let resultSource = FutureSource<Value, NewError>(state: .Unresolved, originalSource: self.source, warnUnresolvedDeinit: true)
  ...
  public func flatMapError<NewError>(transform: Error -> Promise<Value, NewError>) -> Promise<Value, NewError> {
    let resultSource = FutureSource<Value, NewError>()
  ...
  public func mapResult<NewValue, NewError>(transform: Future<Value, Error> -> Future<NewValue, NewError>) -> Promise<NewValue, NewError> {
    let resultSource = FutureSource<NewValue, NewError>(state: .Unresolved, originalSource: self.source, warnUnresolvedDeinit: true)
  ...
  public func flatMapResult<NewValue, NewError>(transform: Future<Value, Error> -> Promise<NewValue, NewError>) -> Promise<NewValue, NewError> {
    let resultSource = FutureSource<NewValue, NewError>()
